...
const char* mqtt_server = "10.107.18.227";
...
void setup()
{
  ...
  Ro = MQCalibration(MQ_PIN);                       //Calibrating the gas sensor. Please make sure the sensor is in clean air 
...
void loop()
{
  if (client.connect(clientID)) {
    ...
    client.publish(gas,String(sensorValue).c_str());
    client.publish(LPG,String(lpg).c_str());
    client.publish(CO,String(co).c_str());
    client.publish(SMOKE,String(smoke).c_str());
  }
  else {
